...
String photoName = pdao.getProductPhoto(productId);
String productName = pdao.getProductName(productId);

/* 총 결제금액 구하기 */
int total= cnt*price;
double vat= (Math.round(total*0.11));
...
<form action="order/orderdirect/orderoneaction.jsp" method="post" id="frm">
...
<table class="table" style="width: 800px; margin-left: 200px;">
		...
		<tr>
			...
			src="<%=root %>/image/AppleProduct_img/<%=photoName%>">
			</div>
			</td>
			<td><div style="margin-top: 80px; font-weight: bold;"><%=productName %></div></td>
			<td><div style="margin-top: 80px; font-weight: bold;"><%=cnt %></div></td>
			<td align="right" style="width: 300px;"><div style="margin-top: 80px; font-weight: bold;">₩<%=dfm.format(price) %></div></td>					
		</tr>
